...
(function() {
  ...
  const getConfig = () => {
    const script = document.getElementById('plubot-script') || document.querySelector('script[data-bot-id]');
    if (!script) {
  ...
  const createStyles = (config) => {
    ...
    const autoThemeCSS = config.theme === 'auto' 
      ? `
  ...
  const createWidgetHTML = (config) => {
    ...
    if (config.showBranding) {
      ...
      branding.innerHTML = 'Powered by <a href="https://plubot.com" target="_blank" rel="noopener">Plubot</a>';
    }
