...
const FlujosCard = ({ hookValues = {} }) => {
  ...
  return (
    <motion.div className='about-card glass holographic-card'>
      ...
      >
        Los <strong className='highlight-text'>flujos de datos</strong> son las
        venas del Pluniverse. Conecta, visualiza y gestiona tus Plubots.
      </motion.p>
      ...
      >
        Desde un panel central, orquesta la sinfonía de tus creaciones y
      ...
      >
        ...
        <span className='button-text'>
          {showMore ? 'CERRAR STREAM' : 'ABRIR STREAM'}
        </span>
        <div className='button-icon'>{showMore ? '×' : '▶'}</div>
      </motion.button>

      <AnimatePresence>
        {showMore && <FlujosExpansion showMore={showMore} />}
      </AnimatePresence>
    </motion.div>
  );
};
