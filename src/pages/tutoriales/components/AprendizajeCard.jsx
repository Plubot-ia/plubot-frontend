...
const CyberpunkButton = ({ showMore, onClick, variants }) => (
  ...
  >
    ...
    <span className='button-text'>
      {showMore ? 'CERRAR CONEXIÓN' : 'INICIAR CONEXIÓN'}
    </span>
...
const AprendizajeCard = ({ hookValues = {} }) => {
  ...
  return (
    <motion.div className='about-card glass holographic-card'>
      ...
      >
        El <strong className='highlight-text'>aprendizaje</strong> es la clave
        para la evolución. Tu Plubot se adapta a ti, no al revés.
      </motion.p>
      ...
      >
        Conéctalo a tus fuentes de datos y observa cómo transforma información
        en acción inteligente.
      </motion.p>
      ...
      <AnimatePresence>
        {showMore && (
          <AprendizajeExpansion
            variants={hookValues.expandableContentVariants}
          />
        )}
      </AnimatePresence>
    </motion.div>
