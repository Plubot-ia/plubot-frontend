...
const ContactHeader = memo(({ titleControls }) => (
  <>
    ...
    >
      Transmite tu señal al <span className='highlight-text'>Pluniverse</span> y
      descubre cómo <span className='highlight-text'>Plubot</span> impulsa tu
      visión con IA de vanguardia.
    </motion.p>
  </>
));
        ...
        {Array.from({ length: 5 }).map((_, index) => (
          <div
            ...
            className={`connection-node ${
              connectionStrength >= (index + 1) * 20 ? 'active' : ''
            }`}
          />
        ))}
      </div>
...
const Contact = () => {
  ...
  const handleSubmit = useMemo(
    () => createFormSubmitHandler(formData, setFormMessage),
    [formData],
  );
