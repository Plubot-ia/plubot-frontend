...
const Profile = () => {
  ...
  useEffect(() => {
    ...
    const loadProfile = async () => {
      try {
        ...
        logger.error('Error detallado al cargar el perfil:', error);
        setNotification({
          message:
            'Error al cargar el perfil. Por favor, inténtalo de nuevo más tarde.',
          type: 'error',
  ...
  useEffect(() => {
    ...
    // La dependencia setRecentActivities es estable,
    // por lo que este efecto se ejecuta solo una vez.
  }, [setRecentActivities]);
  ...
  useEffect(() => {
    ...
    if (showAchievementUnlocked) {
      timer = setTimeout(() => setShowAchievementUnlocked(false), 5000);
    }
        ...
        <div className='profile-content'>
          ...
          {activeTab === 'integrations' && (
            <IntegrationsSection
              user={user}
              showNotification={showNotification}
              // Pasa otras props necesarias, como navigate si es necesario
              // para modales de creación/edición
            />
          )}
        </div>
