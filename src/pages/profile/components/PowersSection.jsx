...
const PowersSection = ({ user, setUser, showNotification, navigate }) => {
  ...
  const handleAddPower = async (e) => {
    ...
    try {
      ...
      const response = await axiosInstance.post('/api/auth/profile/powers', { powerId: newPower });
      ...
      console.error('[PowersSection] Error al agregar poder:', error);
  ...
  const handleRemovePower = async (powerId) => {
    try {
      ...
      const response = await axiosInstance.delete('/api/auth/profile/powers', {
        data: { powerId },
      });
      ...
      console.error('[PowersSection] Error al eliminar poder:', error);
      if (error.response?.status === 401) {
        localStorage.removeItem('access_token');
        showNotification('Sesión expirada. Por favor, inicia sesión nuevamente.', 'error');
        navigate('/login');
