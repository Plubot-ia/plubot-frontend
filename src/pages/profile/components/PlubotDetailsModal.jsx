  ...
  <div className='modal-flows-container'>
    ...
    <ul className='modal-flows-list'>
      {flows.map((flow) => (
        <li
          key={
            flow.id || `flow-${flow.intent}-${flow.user_message || 'default'}`
          }
      ...
      {edges.map((edge) => (
        ...
        >
          <div className='modal-edge-description'>
            <strong>Conexión:</strong> Desde nodo {edge.source} hacia nodo{' '}
            {edge.target}
          </div>
        </li>
      ))}
  ...
  return (
    <div className='modal-available-power-container'>
      ...
      <p className='modal-available-power-text'>
        ...
        para desbloquear la capacidad de {available.description.toLowerCase()}!
      </p>
    </div>
  );
};
