...
export const exchangeCodeForToken = async (code, state, handleAuthSuccess) => {
  ...
  try {
    ...
    logger.warn(
      'La autenticación con el backend falló. Usando usuario simulado.',
      error,
    );
...
export const processToken = async (
  ...
  try {
    ...
    handleAuthError(
      'El token no es válido o ha expirado. Por favor, intenta de nuevo.',
    );
  }
};
...
export const processAuthentication = async (
  ...
  if (error) {
    return handleAuthError(
      params.get('error_description') || `Error: ${error}`,
    );
  }
