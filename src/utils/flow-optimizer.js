...
function _compareEdgeProperties(normalizedOld, normalizedNew) {
  return (
    normalizedOld.label !== normalizedNew.label ||
    normalizedOld.type !== normalizedNew.type
  );
}
...
function _calculateChangeStats({
  currentNodes,
  currentEdges,
  nodeChanges,
  edgeChanges,
...
const _validateStartEndNodes = ({ nodes, edges, errors, warnings }) => {
  ...
  const nodesWithoutOutgoingEdges = _validateNodeOutgoingEdges(
    nodes,
    edges,
    warnings,
  );
