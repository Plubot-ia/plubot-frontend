...
export const hasNodeChanged = (oldNode, newNode) => {
  ...
  const commonProps = ['label', 'message', 'options', 'conditions'];
...
export const hasEdgeChanged = (oldEdge, newEdge) => {
  ...
  // Comparar conexiones
  if (normalizedOld.source !== normalizedNew.source ||
  ...
  if (normalizedOld.style && normalizedNew.style) {
    if (normalizedOld.style.strokeWidth !== normalizedNew.style.strokeWidth ||
        normalizedOld.style.stroke !== normalizedNew.style.stroke) {
      return true;
    }
  } else if ((normalizedOld.style && !normalizedNew.style) ||
 ...
 */
export const calculateFlowDiff = (prevState, currentState) => {
  ...
  const prevEdgesMap = new Map(prevEdges.map(edge => [edge.id, edge]));
  ...
  currentEdges.forEach(edge => {
    const prevEdge = prevEdgesMap.get(edge.id);
    if (!prevEdge) {
...
export const validateFlow = (flow) => {
  ...
  if (startNodes.length > 1) {
    warnings.push(`El flujo tiene ${startNodes.length} nodos de inicio, lo que puede causar comportamientos inesperados`);
  }
  ...
  const disconnectedNodes = nodes.filter(node =>
    !connectedNodes.has(node.id) && node.type !== 'start' && node.type !== 'end',
  );
  ...
  const nodesWithoutOutgoingEdges = nodes.filter(node =>
    node.type !== 'end' && !nodesWithOutgoingEdges.has(node.id),
  );

  if (nodesWithoutOutgoingEdges.length > 0) {
    warnings.push(`Hay ${nodesWithoutOutgoingEdges.length} nodos sin conexiones de salida (excepto nodos de fin)`);
  }
  ...
  decisionNodes.forEach(node => {
    ...
    if (outgoingEdges.length < 2) {
      warnings.push(`El nodo de decisiu00f3n "${node.data?.label || node.id}" tiene menos de 2 salidas`);
    }
  });
