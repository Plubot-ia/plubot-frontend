...
export const validateNode = (node, nodes = [], edges = []) => {
  if (!node || !node.type) {
    console.error('[validateNode] Node o node.type es undefined:', node);
  ...
  if (!validationRules) {
    console.warn(`[validateNode] No hay reglas de validación para el tipo de nodo '${node.type}'. Se asume válido.`);
  ...
  if (nodes.length > 0 && edges.length > 0) {
    ...
    if (validationRules.maxOutDegree !== undefined && outDegree > validationRules.maxOutDegree) {
      console.warn(`Nodo '${node.data.label || node.id}' (tipo: ${node.type}) excede el máximo de ${validationRules.maxOutDegree} conexiones de salida.`);
    ...
    if (validationRules.minOutDegree !== undefined && outDegree < validationRules.minOutDegree) {
      console.warn(`Nodo '${node.data.label || node.id}' (tipo: ${node.type}) requiere al menos ${validationRules.minOutDegree} conexiones de salida.`);
    ...
    if (validationRules.maxInDegree !== undefined && inDegree > validationRules.maxInDegree) {
      console.warn(`Nodo '${node.data.label || node.id}' (tipo: ${node.type}) excede el máximo de ${validationRules.maxInDegree} conexiones de entrada.`);
    ...
    if (validationRules.minInDegree !== undefined && inDegree < validationRules.minInDegree) {
      console.warn(`Nodo '${node.data.label || node.id}' (tipo: ${node.type}) requiere al menos ${validationRules.minInDegree} conexiones de entrada.`);
...
export const validateConnection = (connection, nodes, edges) => {
  ...
  if (!sourceNode || !targetNode) {
    console.error('validateConnection: Nodo origen o destino no encontrado.');
    return false;
  }
  ...
  if (sourceValidationRules) {
    ...
    if (sourceValidationRules.maxOutDegree !== undefined && outDegree >= sourceValidationRules.maxOutDegree) {
      ...
      console.warn(`El nodo '${sourceNode.data.label}' ya alcanzó su máximo de conexiones de salida.`);
      return false;
    }
    ...
    if (sourceValidationRules.allowedOutgoingConnections && 
      ...
      console.warn(`No se puede conectar un nodo '${sourceNode.type}' a un nodo '${targetNode.type}'.`);
      return false;
    }
  }
  ...
  if (targetValidationRules) {
    ...
    if (targetValidationRules.maxInDegree !== undefined && inDegree >= targetValidationRules.maxInDegree) {
      ...
      console.warn(`El nodo '${targetNode.data.label}' ya alcanzó su máximo de conexiones de entrada.`);
      return false;
    }
  }
