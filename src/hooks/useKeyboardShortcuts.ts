...
function parseShortcut(pattern: string): ParsedShortcut {
  ...
  return {
    key,
    requiresMod: parts.includes('mod') || parts.includes('ctrl') || parts.includes('cmd') || parts.includes('meta'),
...
function eventMatches(event: KeyboardEvent, spec: ParsedShortcut): boolean {
  ...
  if (spec.requiresMod !== modPressed) return false;
      ...
      for (const { spec, handler } of parsedShortcuts) {
        if (eventMatches(event, spec)) {
          try {
            ...
            console.error('[useKeyboardShortcuts] Error in handler for', spec, err);
          }
