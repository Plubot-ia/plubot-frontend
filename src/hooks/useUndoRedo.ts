...
interface UndoRedoStoreSlice {
  ...
  setHistory: (
    updater: HistoryState | ((history: HistoryState) => HistoryState),
  ) => void;
  undo: () => void;
  redo: () => void;
}
...
export const useUndoRedo = (): UseUndoRedoReturn => {
  ...
  const { history, setHistory, undoAction, redoAction } = useFlowStore(
    selector,
    shallow,
  );
  ...
  const canUndo = useCallback(() => {
    return (
      (useFlowStore.getState() as UndoRedoStoreSlice).history.past.length > 0
    );
  }, []);

  const canRedo = useCallback(() => {
    return (
      (useFlowStore.getState() as UndoRedoStoreSlice).history.future.length > 0
    );
  }, []);
