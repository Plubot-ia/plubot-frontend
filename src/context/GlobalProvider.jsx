...
export const GlobalProvider = ({ children }) => {
  ...
  const showNotification = (message, type = 'info', duration = 3000) => {
    ...
    // Log para depuración
    console.log('[GlobalProvider] Notificación: ' + message + ' (' + type + ')');
  ...
  const clearAllNotifications = () => {
    console.log('[GlobalProvider] Limpiando todas las notificaciones');
    setNotifications([]);
  };
  ...
  React.useEffect(() => {
    ...
    const handleByteMessage = (event) => {
      ...
      console.log('[GlobalProvider] Evento byte-message recibido:', message, status);
  ...
  const openModal = (modalName) => {
    console.log('[GlobalProvider] Abriendo modal: ' + modalName);
    
    // Verificar que el modal existe
    if (!activeModals.hasOwnProperty(modalName)) {
      console.error('[GlobalProvider] Error: Modal "' + modalName + '" no existe');
      return false;
    }
  ...
  const closeModal = (modalName) => {
    console.log('[GlobalProvider] Cerrando modal: ' + modalName);
    ...
    try {
      ...
      console.error('[GlobalProvider] Error al disparar evento global de cierre:', e);
    }
  ...
  const setGlobalByteMessage = (message, type = 'info') => {
    console.log('[GlobalProvider] ByteMessage: ' + message + ' (' + type + ')');
    ...
    try {
      ...
      console.error('[GlobalProvider] Error al disparar evento ByteMessage:', e);
    }
  };
  
  // Inicializar la función setByteMessage globalmente
  useEffect(() => {
    // Guardar la función original si existe
    const originalSetByteMessage = window.setByteMessage;
    
    // Redefinir setByteMessage globalmente
    window.setByteMessage = setGlobalByteMessage;
    
    // Manejar errores
    const errorHandler = (event) => {
      if (event.error && event.error.message && event.error.message.includes('setByteMessage is not a function')) {
        console.warn('[GlobalProvider] Error detectado: setByteMessage is not a function');
        window.setByteMessage = setGlobalByteMessage;
    ...
    return () => {
      ...
      window.setByteMessage = originalSetByteMessage;
  ...
  useEffect(() => {
    const handleFlowDataUpdate = (event) => {
      try {
        ...
        console.error('[GlobalProvider] Error actualizando datos del flujo:', error);
      }
    };
