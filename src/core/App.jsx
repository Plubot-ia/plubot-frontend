...
function App() {
  // Inicializar función global setByteMessage para compatibilidad
  useEffect(() => {
    // Prevenir errores de "setByteMessage is not a function"
    if (typeof window.setByteMessage !== 'function') {
      window.setByteMessage = (message, type = 'info') => {
        console.log(`[GlobalSetByteMessage] ${message} (${type})`);
        // Disparar evento para que lo capture el GlobalProvider
        try {
    ...
    window.closeAllModals = () => {
      try {
        window.dispatchEvent(new CustomEvent('plubot-close-modal', { 
          detail: { modal: 'all', timestamp: Date.now() } 
        }));
      } catch (e) {}
    };
  }, []);
  ...
  return (
    <GlobalProvider>
      <BrowserRouter>
        <AppWrapper />
        {/* Renderizar el contenedor de modales fuera de las rutas para que esté siempre disponible */}
        <ModalContainer />
      </BrowserRouter>
    </GlobalProvider>
  );
