  ...
  const updateExistingPlubot = useCallback(
    async (id, payload) => {
      try {
        ...
        const newPlubots =
          user?.plubots?.map((p) => (p.id === id ? updatedPlubot : p)) || [];
        ...
        setMessageText(
          `Error al actualizar el Plubot: ${error.message || 'Error desconocido'}`,
        );
        return false;
      }
    },
    ...
    switch (activeSection) {
      ...
      case 'preview': {
        ...
        try {
          ...
          await (isEditMode
            ? updateExistingPlubot(plubotId, payload)
            : createNewPlubot(payload));
        } catch (error) {
