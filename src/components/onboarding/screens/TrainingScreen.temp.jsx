...
const TrainingScreen = () => {
  ...
  useEffect(() => {
    const loadPlubotData = async () => {
      ...
      try {
        if (!plubotIdFromUrl) {
          console.log('No se proporcionó un ID de Plubot. Inicializando con datos por defecto.');
          // En lugar de redirigir, inicializamos con datos por defecto
          ...
          setState((prev) => ({
            ...prev,
            isLoading: false,
            isDataLoaded: true, // Crucial para renderizar el editor
            byteMessage: '🎉 Nuevo Plubot creado. Comienza a diseñar tu flujo.',
          }));
        ...
        if (response.status === 'success' && response.plubot) {
          ...
          // Declaramos edges con let para permitir su reasignación más adelante
          let edgesData = Array.isArray(plubot.edges) ? plubot.edges : [];
          
          console.log(`Cargando datos del Plubot: ${flows.length} nodos y ${edgesData.length} aristas`);

          if (flows.length === 0) {
            console.log('No hay flujos para cargar, inicializando con nodos por defecto');
            setNodes(initialNodes);
            setEdges(initialEdges);
            updatePlubotData({
              id: plubotIdFromUrl,
            ...
            setState((prev) => ({
              ...prev,
              isDataLoaded: true,
              byteMessage: '🔄 Inicializando editor con datos por defecto.',
            }));
            lastSignificantChange.current = { nodes: [...initialNodes], edges: [...initialEdges] };
            return;
          }
...
const newEdges = edges.map((edge, index) => {
              try {
                ...
                // Crear un objeto nuevo para la arista en lugar de modificar el original
                const newEdge = { ...edge };
                
                // Extraer o generar handles válidos
                let effectiveSourceHandle = edge.sourceHandle || 'default';
                let effectiveTargetHandle = edge.targetHandle || null; // Null es válido para targetHandle
                
                // Detectar y procesar sourceHandle con formato JSON serializado
                if (typeof effectiveSourceHandle === 'string' && effectiveSourceHandle.startsWith('|||{')) {
                  try {
                    // Extraer el JSON después del prefijo '|||'
                    const jsonStr = effectiveSourceHandle.substring(3);
                    const handleData = JSON.parse(jsonStr);
                    
                    console.log('Procesando sourceHandle con datos JSON:', handleData);
                    
                    // Actualizar el sourceHandle efectivo sin modificar el original
                    effectiveSourceHandle = handleData.sourceHandle || 'default';
                    
                    // Extraer otros datos útiles
                    let updatedSourceId = sourceId;
                    
                    if (handleData.source_original) {
                      // Guardar el ID original para referencia
                      newEdge.sourceOriginal = handleData.source_original;
                      
                      // Intentar actualizar el source con el ID original
                      if (nodeIdMap[handleData.source_original]) {
                        // Actualizar source en la nueva arista y en la variable local
                        const originalSource = handleData.source_original;
                        updatedSourceId = originalSource;
                    ...
                    if (handleData.target_original) {
                      // Guardar el ID original para referencia
                      newEdge.targetOriginal = handleData.target_original;
                      
                      // Intentar actualizar el target con el ID original
                      if (nodeIdMap[handleData.target_original]) {
                        const originalTarget = handleData.target_original;
                        updatedTargetId = originalTarget;
                        console.log(`Usando ID original para target: ${originalTarget}`);
                      } else {
                        console.log(`ID original para target no encontrado: ${handleData.target_original}`);
                      }
                    }
                    
                    // Si hay estilo personalizado, aplicarlo
                    if (handleData.style) {
                      newEdge.style = { ...handleData.style };
                    }
                    
                    // Actualizar los IDs de source y target en la nueva arista
                    newEdge.source = updatedSourceId;
                    newEdge.target = updatedTargetId;
                  } catch (error) {
                    console.error('Error al procesar sourceHandle con formato JSON:', error);
                    effectiveSourceHandle = 'default';
                  }
                }
                
                // Traducir IDs numéricos a IDs de nodo
                let translatedSourceId = newEdge.source;
                
                // Primero intentar con el mapa de IDs existente
                if (nodeIdMap[translatedSourceId]) {
                  translatedSourceId = nodeIdMap[translatedSourceId];
                  console.log(`Traduciendo source ${newEdge.source} a ${translatedSourceId}`);
                } 
                // Si no se encuentra, usar directamente los IDs de nodo generados por el sistema
                else {
                  // Intentar mapear directamente a los IDs de nodo (node-0, node-1, etc.)
                  const sourceIndex = parseInt(translatedSourceId.split('-')[1] || '0', 10) - 1; // Convertir a base 0
                  
                  if (sourceIndex >= 0 && sourceIndex < nodes.length) {
                    // Usar directamente el ID de nodo en esa posición (node-0, node-1, etc.)
                    translatedSourceId = `node-${sourceIndex}`;
                    console.log(`Mapeando source directamente a ID de nodo: ${newEdge.source} -> ${translatedSourceId}`);
                    // Agregar al mapa para futuras referencias
                    nodeIdMap[newEdge.source] = translatedSourceId;
                  }
                }
                
                let translatedTargetId = newEdge.target;
                
                // Primero intentar con el mapa de IDs existente
                if (nodeIdMap[translatedTargetId]) {
                  translatedTargetId = nodeIdMap[translatedTargetId];
                  console.log(`Traduciendo target ${newEdge.target} a ${translatedTargetId}`);
                }
                // Si no se encuentra, usar directamente los IDs de nodo generados por el sistema
                else {
                  // Intentar mapear directamente a los IDs de nodo (node-0, node-1, etc.)
                  const targetIndex = parseInt(translatedTargetId.split('-')[1] || '0', 10) - 1; // Convertir a base 0
                  
                  if (targetIndex >= 0 && targetIndex < nodes.length) {
                    // Usar directamente el ID de nodo en esa posición (node-0, node-1, etc.)
                    translatedTargetId = `node-${targetIndex}`;
                    console.log(`Mapeando target directamente a ID de nodo: ${newEdge.target} -> ${translatedTargetId}`);
                    // Agregar al mapa para futuras referencias
                    nodeIdMap[newEdge.target] = translatedTargetId;
                  }
                }
                
                console.log(`Usando IDs traducidos para arista: ${translatedSourceId} -> ${translatedTargetId}`);
                // Asegurar que la arista tenga un ID único
                const edgeId = newEdge.id || `edge-${index}-${translatedSourceId}-${translatedTargetId}`;
                
                // Retornar la arista con todos los datos necesarios
                return {
                  id: edgeId,
                  source: translatedSourceId,
                  target: translatedTargetId,
                  sourceHandle: effectiveSourceHandle,
                  targetHandle: effectiveTargetHandle,
                  // Asegurar que tenga un tipo válido
                  type: newEdge.type || 'elite',
                  // Mantener animación si existe
                  animated: newEdge.animated || false,
                  // Mantener etiqueta si existe
                  label: newEdge.label,
                  // Asegurar que tenga un estilo base
                  style: newEdge.style || { stroke: '#00e0ff', strokeWidth: 2 },
                  // Datos adicionales para debugging
                  data: {
                    ...newEdge.data,
                    edgeId: edgeId,
                    originalSource: newEdge.sourceOriginal || translatedSourceId,
                    originalTarget: newEdge.targetOriginal || translatedTargetId,
                  }
                };
