...
const ByteAssistant = ({ simulationMode }) => {
  ...
  const sendToAI = async (userMessage) => {
    ...
    try {
      ...
      if (data.suggestions) {
        console.log('Sugerencias de Byte:', data.suggestions);
      }
    } catch (error) {
  ...
  return (
    <div
      className={`ts-byte-assistant ${isAnimating ? 'ts-byte-animating' : ''} ${
        ...
        <div className="ts-byte-hologram">
          <div className="ts-byte-image-container" onClick={handleImageClick} style={{ cursor: 'pointer' }}>
            <img
              src={getByteImage()}
              alt="Byte asistente"
              className={`ts-byte-image ${isLoading ? 'ts-byte-thinking' : ''}`}
            />
            <div
