...
const CustomMiniMap = () => {
  ...
  return (
    <div className={`custom-minimap ${isUltraMode ? 'ultra-mode' : ''} ${isCollapsed ? 'collapsed' : 'expanded'}`}>
      {/* Renderizado fantasma oculto: procesa la estructura del mapa expandido para testear hipótesis de rendimiento */}
      <div style={{ display: 'none' }}>
        {nodes && nodes.length > 0 && edges && minimapNodes.length > 0 && (
          <>
              ...
              {edges.map(edge => {
                ...
                return (
                  <line
                    ...
                    stroke="#00e0ff" strokeWidth="0.1" opacity="0.1"
                  />
                );
              })}
            </svg>
