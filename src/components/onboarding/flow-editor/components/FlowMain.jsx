...
// Helper: Validar si un edge es válido para renderizado
const isValidEdgeForRender = (edge) => {
  return edge && edge.source && edge.target;
};

// Helper: Sanitizar handles de un edge
...
const FlowMain = ({
  ...
  // Helper: Agregar propiedades de presentación a edge
  const addEdgePresentationProps = (
    edge,
    currentLodLevel,
    currentIsUltraMode,
  ) => {
    return {
      ...edge,
      animated: !currentIsUltraMode && currentLodLevel === LOD_LEVELS.FULL,
      data: {
        ...edge.data,
  ...
  const edgesWithLOD = useMemo(() => {
    const processedEdges = visibleEdges
      .map((edge) => {
        ...
        return addEdgePresentationProps(sanitizedEdge, lodLevel, isUltraMode);
      })
...
/*      const cleanup = initOptimizedFixes({
  ...
  // Helpers para reducir complejidad del useEffect de corrección de nodos
   ...
   */
  useEffect(() => {
    // Validar si los nodos necesitan actualización
