...
const FlowMain = ({
  ...
  const edgesWithLOD = useMemo(() => {
    const processedEdges = visibleEdges
      .map((edge) => {
        // SANITIZACIÓN EN TIEMPO REAL: Corregir handles problemáticos antes del filtro
        // Esto asegura que todos los edges pasen por sanitización, independientemente de su origen
        const sanitizedEdge = { ...edge };

        // Omitir sourceHandle si es inválido (React Flow prefiere ausencia de propiedad)
        const isSourceHandleInvalid =
        ...
        if (isSourceHandleInvalid) {
          delete sanitizedEdge.sourceHandle;
        }

        // Omitir targetHandle si es inválido (React Flow prefiere ausencia de propiedad)
        const isTargetHandleInvalid =
        ...
        if (isTargetHandleInvalid) {
          delete sanitizedEdge.targetHandle;
        }

        // Agregar propiedades de presentación SIN usar spread que restaure handles eliminados
        sanitizedEdge.animated = !isUltraMode && lodLevel === LOD_LEVELS.FULL;
        sanitizedEdge.data = {
          ...sanitizedEdge.data,
      ...
      .filter((edge) => {
        // FILTRO FINAL: Solo rechazar edges completamente inválidos
        // Todos los handles deberían estar sanitizados a este punto
