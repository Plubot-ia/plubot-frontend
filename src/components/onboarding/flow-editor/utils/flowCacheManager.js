...
export const nodePositionCache = {
  ...
  savePositions: (flowId, nodes) => {
    ...
    try {
      ...
      console.warn('[flowCacheManager] Error al guardar posiciones en cache:', error.message);
    }
  },
  ...
  getPositions: (flowId) => {
    ...
    try {
      ...
      console.warn('[flowCacheManager] Error al recuperar posiciones:', error.message);
      return null;
    }
  },
    ...
    flowStateManager.saveTimer = setTimeout(() => {
      ...
      if (flowStateManager.lastSavedState && stateToSave) {
        ...
        if (nodesEqual && edgesEqual) {
          ...
          if (!hasChanges) {
            console.log('[flowCacheManager] Omitiendo guardado redundante, no hay cambios significativos.');
