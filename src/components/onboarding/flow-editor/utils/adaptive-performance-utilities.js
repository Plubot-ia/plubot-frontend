...
const calculateFrameDurations = (frameTimes) => {
  ...
  for (let index = 1; index < frameTimes.length; index++) {
    const hasCurrentFrame = Object.prototype.hasOwnProperty.call(
      frameTimes,
      index,
    );
    const hasPreviousFrame = Object.prototype.hasOwnProperty.call(
      frameTimes,
      index - 1,
    );
    ...
    if (hasCurrentFrame && hasPreviousFrame) {
      const currentFrame =
        frameTimes[Math.max(0, Math.min(index, frameTimes.length - 1))]; // Sanitized index
...
const calculateAverageFPS = (frameDurations) => {
  const avgFrameTime =
    frameDurations.reduce((sum, time) => sum + time, 0) / frameDurations.length;
  return {
...
export function calculateComplexity(nodes = [], edges = [], statsReference) {
  ...
  const baseComplexity = calculateBaseComplexity(
    validNodes.length,
    validEdges.length,
  );
