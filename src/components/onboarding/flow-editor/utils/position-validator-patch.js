...
// Variables a nivel de módulo para el intervalo y el observador
...
const applyPositionValidatorPatch = () => {
  ...
  useFlowStore.setState({
    setReactFlowInstance: (instance) => {
      ...
      const sanitizeInterval = setInterval(() => {
        ...
        if (!useFlowStore.getState().isNodeBeingDragged) {
          sanitizeEdgePaths();
        }
      ...
      positionValidatorInterval = sanitizeInterval;
  ...
  window.addEventListener('beforeunload', () => {
    if (positionValidatorInterval) {
      ...
      positionValidatorInterval = null; // Buena práctica: limpiar la referencia
    }
    if (positionValidatorObserver) {
      positionValidatorObserver.disconnect();
      positionValidatorObserver = null; // Buena práctica: limpiar la referencia
    }
  });
