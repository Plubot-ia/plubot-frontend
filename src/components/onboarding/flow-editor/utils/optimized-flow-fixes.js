...
const removeBlueRectangle = () => {
  ...
  for (const selector of elementsToFix) {
    ...
    for (const element of elements) {
      ...
      if (selector === '.react-flow__pane') {
        ...
        element.style.pointerEvents = 'all'; // Mantener interactividad

        // Método directo para modificar el estilo inline (más efectivo)
        const currentStyle = element.getAttribute('style') || '';
        const styleWithoutBlue = currentStyle.replaceAll(
          /background(-color)?\s*:\s*rgba\(0,\s*89,\s*220[^;]*;?/g,
          '',
        );
        element.setAttribute(
          'style',
          `${styleWithoutBlue}background-color: transparent !important; background: none !important;`,
        );
    ...
    const observer = new MutationObserver((mutations) => {
      for (const mutation of mutations) {
        if (
          ...
          if (
            style.includes('rgba(0, 89, 220') ||
            style.includes('rgb(0, 89, 220')
          ) {
  ...
  const blueElements = [...document.querySelectorAll('*')].filter((element) => {
  ...
  for (const element of blueElements) {
    ...
    if (
      ...
      element.parentElement?.classList.contains('react-flow__pane')
    ) {
  ...
  if (!document.querySelector('style[data-id="remove-blue-fix"]')) {
    customStyle.dataset.id = 'remove-blue-fix';
  ...
  if (!document.querySelector('#pane-fix-overlay')) {
    ...
    if (reactFlowContainer) {
      ...
      reactFlowContainer.append(paneFixOverlay);
    }
  }
};
...
export const cleanup = () => {
  // Limpiar intervalos
  for (const id of instances.intervals) clearInterval(id);
  instances.intervals = [];

  // Desconectar observers
  for (const obs of instances.observers) obs.disconnect();
  instances.observers = [];
