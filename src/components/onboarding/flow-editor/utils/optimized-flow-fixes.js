...
const applyNodeVisibilityFix = () => {
  ...
  // Log condicional solo si está habilitado
  if (CONFIG.enableLogs) {
    console.log(`[NodeFix] Aplicando visibilidad a ${nodes.length} nodos`);
  }
  
  // Importar CSS de eliminación de debug si existe (manejado por import directo en FlowMain.jsx)
  /* try {
    // Verificar si ya se ha importado
    if (!document.querySelector('link[href*="remove-debug-styles.css"]')) {
      const linkElement = document.createElement('link');
      linkElement.rel = 'stylesheet';
      linkElement.href = '../ui/remove-debug-styles.css';
      document.head.appendChild(linkElement);
    }
  } catch (e) {
    console.warn('[NodeFix] No se pudo importar remove-debug-styles.css:', e);
  } */
...
const applyNodeDragFix = () => {
  ...
  // Log condicional
  if (CONFIG.enableLogs) {
    console.log(`[NodeFix] Aplicando fix de arrastre a ${nodes.length} nodos`);
  }
...
const hideControls = () => {
  ...
  if (controls) {
    ...
    if (CONFIG.enableLogs) {
      console.log('[NodeFix] Controles de ReactFlow ocultados');
    }
  }
};
...
const applyAllFixes = () => {
  try {
    ...
    if (CONFIG.enableLogs) {
      console.log(`[AllFixes] Encontrados ${nodes.length} nodos y ${edges.length} aristas`);
    }
    ...
    if (CONFIG.fixes.nodeVisibility) {
      try {
        ...
        console.error('[AllFixes] Error al aplicar fix de visibilidad:', e);
      }
    }
    ...
    if (CONFIG.fixes.nodeDrag) {
      try {
        ...
        console.error('[AllFixes] Error al aplicar fix de arrastre:', e);
      }
    }
    ...
    if (CONFIG.fixes.removeBlueRectangle) {
      try {
        ...
        console.error('[AllFixes] Error al eliminar rectángulo azul:', e);
      }
    }
    ...
    if (CONFIG.fixes.hideControls) {
      try {
        ...
        console.error('[AllFixes] Error al ocultar controles:', e);
      }
    }
    ...
    console.error('[AllFixes] Error general al aplicar fixes:', error);
    return false;
  }
};
