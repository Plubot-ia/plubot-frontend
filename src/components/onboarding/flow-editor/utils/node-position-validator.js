 ...
 */
export const generateValidPosition = (existingNodes = [], preferredPosition = null) => {
  ...
  if (attempts >= MAX_ATTEMPTS) {
    ...
    console.warn('Se agotaron los intentos para encontrar una posición única, usando posición aleatoria');
  }
...
function isDraggingInProgress() {
  try {
    return window && window.__dragInProgress === true;
  } catch (e) {
    return false;
  }
}
...
export const validateNodePositions = (nodes) => {
  // Conditional logging
  if (window.DEBUG_NODE_POSITIONS) {
    console.log('[validateNodePositions] Called. window.__dragInProgress =', window.__dragInProgress, 'isDraggingInProgress() =', isDraggingInProgress());
  }
  ...
  // Crear copias validadas de todos los nodos
  return nodes.map(node => {
    // Si la posición es válida, mantener el nodo sin cambios
    if (isValidPosition(node.position)) {
      return node;
    }
    ...
    // Generar una posición válida
    validatedNode.position = generateValidPosition(
      nodes.filter(n => isValidPosition(n.position)),
    ...
    if (window.DEBUG_NODE_POSITIONS) {
      console.log(`[node-validator] Nodo ${node.id} corregido. Nueva posición:`, validatedNode.position);
    }
 ...
 */
export const createNodeValidatorMiddleware = (set, get) => (args) => {
