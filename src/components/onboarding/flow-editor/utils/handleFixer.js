...
export const normalizeEdgeHandles = (edge) => {
  if (!edge || !edge.id || !edge.source || !edge.target) {
    console.warn('Arista inválida:', edge);
    return null;
  }
  
  // Crear una copia profunda para no modificar el original
  const normalizedEdge = JSON.parse(JSON.stringify(edge));
  
  // Registrar los handles originales para debugging
  const originalSourceHandle = normalizedEdge.sourceHandle;
  const originalTargetHandle = normalizedEdge.targetHandle;
  
  // Obtener handles válidos para source y target
  normalizedEdge.sourceHandle = getValidHandle(normalizedEdge.source, originalSourceHandle || 'default');
  normalizedEdge.targetHandle = getValidHandle(normalizedEdge.target, originalTargetHandle || 'default');
  
  // Asegurar que los IDs de los nodos sean strings
  normalizedEdge.source = String(normalizedEdge.source);
  normalizedEdge.target = String(normalizedEdge.target);
  
  // Si es una arista de bucle, asegurarse de que los handles sean diferentes
  if (normalizedEdge.source === normalizedEdge.target) {
    if (normalizedEdge.sourceHandle === normalizedEdge.targetHandle) {
      // Si son iguales, cambiar el targetHandle para evitar colisiones
      normalizedEdge.targetHandle = `${normalizedEdge.targetHandle}-out`;
    }
  }
  
  // Registrar el cambio para debugging si hubo un cambio real
  if (originalSourceHandle !== normalizedEdge.sourceHandle || 
      originalTargetHandle !== normalizedEdge.targetHandle) {
    console.log(`Handles normalizados para arista ${normalizedEdge.id}:`, {
      antes: { 
        source: edge.source, 
        target: edge.target, 
