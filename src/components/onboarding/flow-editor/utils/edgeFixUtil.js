...
const _ensureEdgesAreVisibleImpl = (edges, isUltraMode, maxAttempts = 5, delay = 300) => {
  ...
  const repairedEdges = edges.filter(edge => {
    ...
    // Si no existen, no debemos incluir esta arista (soluciona aristas sueltas)
    ...
    if (!sourceExists || !targetExists) {
      console.log(`Arista ${edge.id} tiene nodos que no existen (source: ${!!sourceExists}, target: ${!!targetExists})`);
      return false;
    }
