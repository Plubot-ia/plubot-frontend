...
const useDragAndDropManager = (reactFlowWrapperRef, reactFlowInstance, setHasChanges) => {
  ...
  const onDrop = useCallback((event) => {
    ...
    if (typeof nodeInfoString === 'undefined' || !nodeInfoString) {
      console.error('[useDragAndDropManager] Datos del nodo no definidos al soltar');
      return;
    }
    ...
    try {
      ...
      console.error('[useDragAndDropManager] Error al parsear datos del nodo:', e, 'String recibido:', nodeInfoString);
      return;
    }

    if (!nodeInfo || !nodeInfo.type || !nodeInfo.id || !nodeInfo.data || typeof nodeInfo.data.label === 'undefined') {
      console.error('[useDragAndDropManager] Datos del nodo incompletos o malformados después de parsear:', nodeInfo);
      return;
    }

    if (!reactFlowWrapperRef.current || !reactFlowInstance) {
      console.error('[useDragAndDropManager] reactFlowWrapperRef o reactFlowInstance no están disponibles.');
      return;
    }
    ...
    if (newNode.type === 'decision') {
      ...
      setNodes((nds) => nds.concat(newNode, ...optionNodes));
    ...
    setTimeout(() => {
      try {
        ...
        console.error('[useDragAndDropManager] Error al forzar visibilidad en onDrop:', e);
      }
    }, 100);
