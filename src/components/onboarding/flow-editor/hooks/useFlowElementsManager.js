...
const createNodeTypeData = (nodeType, nodeId) => {
  const baseData = {
    ...
    label: (() => {
      if (
        NODE_LABELS &&
        Object.prototype.hasOwnProperty.call(NODE_LABELS, nodeType)
      ) {
        // Acceso seguro para evitar Generic Object Injection Sink
        const safeValue = Object.prototype.hasOwnProperty.call(
          NODE_LABELS,
          nodeType,
        )
        ...
        return (
          safeValue ||
          `Nuevo ${String(nodeType).replaceAll(/[^a-zA-Z0-9]/g, '')}`
        ); // Sanitized
      }
