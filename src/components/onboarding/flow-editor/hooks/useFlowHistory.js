...
const filterNodesNotInAction = (nodes, actionNodes) => {
  ...
  return nodes.filter(
    (node) => !actionNodes.some((actionNode) => actionNode.id === node.id),
  );
};
...
const filterEdgesNotInAction = (edges, actionEdges) => {
  ...
  return edges.filter(
    (edge) => !actionEdges.some((actionEdge) => actionEdge.id === edge.id),
  );
};
