...
const BackgroundScene = ({ isUltraMode = false }) => {
  ...
  useEffect(() => {
    ...
    // Forzar reflow para asegurar que el canvas sea visible
    setTimeout(() => {
  ...
  if (mode === 'ultra') {
    // Renderizado en modo Ultra Rendimiento - con un fondo estático ligero en lugar de nulo
    return (
      <div className="ts-background-scene-container" aria-hidden="true" style={{
        position: 'absolute',
        top: 0,
        left: 0,
        width: '100%',
        height: '100%',
        backgroundColor: 'rgba(20, 28, 40, 0.4)', // Fondo sutil para modo ultra
        zIndex: 0,
        pointerEvents: 'none'
      }} />
    );
