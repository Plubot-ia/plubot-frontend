...
const CustomMiniMap = ({
  ...
  toggleMiniMap,
  setByteMessage: propertySetByteMessage,
    ...
    (event) => {
      ...
      try {
        ...
        const newX = sanitizeNumber(
          viewport.x - deltaX / viewport.zoom / scale,
          viewport.x,
        );
        const newY = sanitizeNumber(
          viewport.y - deltaY / viewport.zoom / scale,
          viewport.y,
        );
    ...
    try {
      ...
      if (isExpanded) {
        try {
          ...
          const safeScale =
            typeof scale === 'number' && scale !== 0 && isFinite(scale)
              ? scale
              : 1;
