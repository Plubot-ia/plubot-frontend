  ...
  ({
    ...
    const handleTestRequest = async () => {
      ...
      try {
        ...
        setNodes((nds) =>
            nds.map((n) => (n.id === id ? { ...n, data: updatedNodeData } : n))
        );
    ...
    return (
      <div
        ref={nodeRef}
        className={`ts-http-request-node ts-node-base ${getStatusClass()} ${isHovered ? 'ts-hovered' : ''} ${selected ? 'ts-selected' : ''} ${isCollapsed ? 'ts-collapsed-node' : ''}`}
        onClick={handleClick}
        onContextMenu={handleContextMenu}
        onMouseEnter={() => setIsHovered(true)}
        onMouseLeave={() => setIsHovered(false)}
        style={{ width: data.width, height: isCollapsed ? 'auto' : data.height }} 
      >
