...
const DecisionNodeHandles = ({ 
  ...
  // Se ejecuta cada vez que el NÚMERO de salidas cambia o el modo ultra se alterna.
  // Esto asegura que React Flow redibuje la cantidad correcta de handles.
  useEffect(() => {
    if (nodeId) {
      // Un delay (250ms) mayor que la animación del nodo (200ms) asegura que
      // las aristas se recalculen cuando el nodo ya esté en su posición final.
      const timer = setTimeout(() => updateNodeInternals(nodeId), 250);
      return () => clearTimeout(timer);
    }
  }, [nodeId, outputs.length, isUltraPerformanceMode, updateNodeInternals]);
