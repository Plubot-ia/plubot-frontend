...
// Hook useNode eliminado ya que implementamos la lógica directamente
...
const MessageNode = memo(({ data = {}, isConnectable, selected, id, setNodes, isUltraPerformanceMode, performanceMode }) => {
  ...
  const [message, setMessage] = useState(safeData.message || DEFAULT_MESSAGE);
  ...
  const saveChanges = useCallback(() => {
    // Usar setNodes directamente para actualizar los datos del nodo
    setNodes((nds) =>
      nds.map((node) =>
        node.id === id
          ? {
