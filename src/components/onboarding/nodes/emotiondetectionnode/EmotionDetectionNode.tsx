...
const EmotionDetectionNode: React.FC<EmotionDetectionNodeProps> = ({ id, data, selected }) => {
  ...
  const getEmotionColor = () => {
    ...
    switch (data.detectedEmotion) {
      ...
      case 'frustrated': return '#f59e0b';
      case 'surprised': return '#fbbf24';
      default: return '';
    }
  };
        ...
        <div className="emotion-node__content">
          ...
          <div className="emotion-node__body">
            ...
            {data.detectedEmotion && (
              <div className="emotion-node__preview">
                Emoción detectada: <span className="emotion-label">{data.detectedEmotion} {EMOTION_EMOJIS[data.detectedEmotion]}</span>
              </div>
            )}
          </div>
        </div>
