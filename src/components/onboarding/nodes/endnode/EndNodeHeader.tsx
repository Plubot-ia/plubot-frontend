import React, { useRef, useEffect, ChangeEvent, KeyboardEvent as ReactKeyboardEvent, MouseEvent as ReactMouseEvent } from 'react';
...
const EndNodeHeader: React.FC<EndNodeHeaderProps> = ({
  label,
  status,
  customIcon,
  isCollapsed,
  ...
  return (
    <div className={cn('end-node-header', { 'editing-label': isEditingLabel })}>
      ...
      <div className="flex items-center flex-grow mr-2 overflow-hidden" onDoubleClick={canEdit ? onStartEditingLabel : undefined}>
      ...
      <div className="end-node-title-container" onDoubleClick={handleLabelDoubleClick}>
        {isEditingLabel ? (
          <div className="end-node-label-edit-container">
            ...
            />
            <button onClick={onLabelSubmit} className="end-node-label-edit-button submit" aria-label="Confirmar etiqueta"><Check size={16}/></button>
            <button onClick={onLabelCancel} className="end-node-label-edit-button cancel" aria-label="Cancelar edición"><X size={16}/></button>
          </div>
        ) : (
        ...
        {status && !isEditingLabel && (
          <Tooltip content={`Estado: ${status}`} position="top">
            <span className={`end-node-status-badge status-${status.toLowerCase()}`}>
              {status}
            </span>
          </Tooltip>
        )}
