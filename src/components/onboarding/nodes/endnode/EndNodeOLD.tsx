...
const EndNode: React.FC<EndNodeProperties> = memo<EndNodeProperties>(
  ({
    id: nodeId,
    data,
    selected,
    ...
    const updateNodeData = useCallback(
      (nodeIdToUpdate: string, changes: Partial<EndNodeData>) => {
        setNodes((nds) =>
          nds.map((n) => {
    ...
    const handleLabelSubmit = useCallback(() => {
      if (editingLabelValue.trim() === '') {
        ...
        effectiveTrackEvent?.('nodeLabelUpdated', {
          nodeId,
          type: NODE_CONFIG.TYPE,
          newLabel: editingLabelValue,
        });
      }
      finishEditingLabel();
    }, [
    ...
    const handleDuplicateNode = useCallback(() => {
      ...
      const newNodeData = {
        ...data,
        label: `${data.label || NODE_CONFIG.DEFAULT_LABEL} (Copia)`,
      };
    ...
    const nodeStyle: React.CSSProperties = useMemo(() => {
      const style: React.CSSProperties = {
        borderColor: selected
          ...
          : NODE_CONFIG.COLORS.BORDER_DEFAULT,
        border: selected
          ? '2px solid var(--color-primary-500, #007bff)'
          : '1px solid var(--color-neutral-300, #bbb)',
      ...
      if (isCollapsed) {
        ...
        style.width = data.width
          ? `${data.width}px`
          : `${NODE_CONFIG.DIMENSIONS.WIDTH}px`;
    ...
    return (
      <>
        <NodeResizer
          ...
          minHeight={
            NODE_CONFIG.DIMENSIONS.HEADER_HEIGHT +
            NODE_CONFIG.DIMENSIONS.MIN_CONTENT_HEIGHT
          }
          onResize={(_event, newDimensions) => {
            ...
            effectiveTrackEvent?.('nodeResized', {
              nodeId,
              type: NODE_CONFIG.TYPE,
              width: newDimensions.width,
              height: newDimensions.height,
        ...
        <div
          ...
          onDoubleClick={() => {
            if (!isUltraPerformanceMode) startEditingLabel();
          }}
          role='button'
          tabIndex={0}
            ...
            {EndNodeHeader === undefined ? (
              ...
              <EndNodeHeader
                ...
                customIcon={React.createElement(
                  getCustomIcon(data.customIconComponent),
                )}
                isCollapsed={isCollapsed}
                isEditingLabel={isEditingLabel}
                editingLabelValue={editingLabelValue}
                labelInputRef={labelInputReference}
