    ...
    (event: React.MouseEvent) => {
      ...
      showContextMenu({
        ...
        items: [
          ...
          {
            label: 'Eliminar Nodo',
            // eslint-disable-next-line @typescript-eslint/no-unsafe-return, @typescript-eslint/no-unsafe-call, @typescript-eslint/no-unsafe-member-access -- Zustand store deleteNode in mixed JS/TS codebase
            action: () => useFlowStore.getState().deleteNode(id),
          },
        ],
      });
    },
    [id, getNode, showContextMenu],
  );
