...
function _processJsonSourceHandle(sourceHandle) {
  if (typeof sourceHandle === 'string' && sourceHandle.startsWith('|||{')) {
    try {
      ...
      return { sourceHandle: 'default', targetHandle: undefined };
    }
  }
...
function _adjustHandlePosition(sourceHandleId, sourceX) {
  ...
  const adjustedSourceX =
    typeof sourceX === 'number' ? sourceX + offsetX : sourceX;
...
const CustomEdge = (properties) => {
  ...
  const {
    sourceHandle: processedSourceHandle,
    targetHandle: processedTargetHandle,
  ...
  const { sourceHandle: adjustedSourceHandle, sourceX: adjustedSourceX } =
    _adjustHandlePosition(sourceHandleId, sourceX);
