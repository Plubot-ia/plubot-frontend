...
const callGrokApi = async (
  ...
  console.log('[useAINode] Realizando llamada al backend para Grok API:', { prompt, temperature, maxTokens, systemMessage });
  try {
    ...
    console.error('[useAINode] Error en la llamada al backend:', error);
    throw error;
  }
};
...
export const useAINode = ({ id, data }: UseAINodeProps) => {
  ...
  const updateNodeData = useCallback((newData: Partial<AiNodeData>) => {
    setNodes((nds) =>
      nds.map((node) => {
        if (node.id === id) {
          // Creamos un nuevo objeto data para asegurar la reactividad
