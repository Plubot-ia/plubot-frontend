...
const ImportExportModal = ({
  ...
  setImportData,
  setExportMode,
  setByteMessage: propSetByteMessage, // Renombramos para evitar conflictos
  ...
  const notifyMessage = (message, type = 'info') => {
    ...
    if (propSetByteMessage) {
      try {
        propSetByteMessage(message);
      } catch (error) {
        console.warn('[ImportExportModal] Error usando setByteMessage como prop:', error);
      }
    }
  };
  const exportFlow = () => {
    try {
      ...
      notifyMessage(`¡Flujo exportado con éxito! Se guardó como ${downloadLink.download}`, 'success');
    } catch (error) {
      console.error('Error al exportar flujo:', error);
      notifyMessage('Error al exportar el flujo. Por favor, intenta de nuevo.', 'error');
    }
  };
  ...
  const importFlow = () => {
    try {
      if (!importData.trim()) {
        notifyMessage('Por favor, ingresa datos de flujo para importar.', 'warning');
        return;
      }
      ...
      if (!hasStart || !hasMessage) {
        notifyMessage('Precaución: El flujo importado puede estar incompleto. Asegúrate de tener nodos de inicio y mensaje.', 'warning');
      }
