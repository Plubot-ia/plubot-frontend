...
function _formatSaveTime(saveTime) {
  return saveTime ? `${saveTime.toFixed(0)}ms` : 'N/A';
}

// Helper function to format memory usage
function _formatMemoryUsage(memoryUsage) {
  if (memoryUsage === undefined || memoryUsage === null) {
    return 'N/A';
  }

  if (memoryUsage < 1024) {
    return `${memoryUsage.toFixed(0)} KB`;
  }
...
// Helper function to update memory usage
const _updateMemoryUsage = (nodes, edges, setMemoryUsage) => {
...
const _createFormatters = ({
  ...
  formatSaveTime: () => _formatSaveTime(saveTime),
  formatMemoryUsage: () => _formatMemoryUsage(memoryUsage),
    ...
    <div style={{ padding: '12px 16px', borderTop: '1px solid #333' }}>
      ...
      >
        ...
        <div>
          ...
          <div style={{ fontSize: '14px', color: '#00c3ff' }}>
            {calculateEfficiency()}%
          </div>
        </div>
      </div>
...
const PerformanceStats = () => {
  ...
  useEffect(() => {
    _updateMemoryUsage(nodes, edges, setMemoryUsage);
  }, [nodes, edges]);
  ...
  return (
    ...
    >
      ...
      {_renderExpandedContent({
        ...
        formatSaveTime: formatters.formatSaveTime,
        formatMemoryUsage: formatters.formatMemoryUsage,
