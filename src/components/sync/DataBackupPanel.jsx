...
const DataBackupPanel = () => {
  ...
  const handleExportAll = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setStatus({
          type: 'error',
          message: result.message || 'Error al exportar plubots',
        });
      }
    } catch (error) {
      setStatus({
  ...
  const handleImport = async (e) => {
    ...
    const [file] = fileInput.files; // Usar destructuring
    setIsLoading(true);
    setStatus({ type: '', message: '' });
    ...
    try {
      ...
      if (result.success) {
        ...
        setStatus({
          type: 'error',
          message: result.message || 'Error al importar plubots',
        });
      }
    } catch (error) {
      setStatus({
  ...
  return (
    <div className='data-backup-panel'>
      ...
      <p className='panel-description'>
        Exporta tus plubots como archivo JSON para tener un respaldo adicional o
        importa plubots previamente exportados.
      </p>
      ...
      <div className='backup-actions'>
        <div className='action-card export-card'>
          ...
          <p>
            Descarga todos tus plubots como archivo JSON para guardarlos de
            forma segura.
          </p>
          <button
            ...
            disabled={
              isLoading || !user || !user.plubots || user.plubots.length === 0
            }
          >
      ...
      <div className='backup-tips'>
        ...
        <ul>
          <li>
            Exporta regularmente tus plubots para tener respaldos actualizados.
          </li>
          <li>
            Guarda los archivos de respaldo en múltiples ubicaciones (nube,
            disco externo, etc.).
          </li>
          <li>
            Los respaldos incluyen toda la configuración y estructura de tus
            plubots.
          </li>
          <li>
            Al importar plubots existentes, puedes elegir sobrescribir o
